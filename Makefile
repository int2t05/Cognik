...
server-build: ## 编译 Go 后端
	cd server && $(GO_CMD) build -o bin/opsmind-server ./cmd/main.go
...
docker-server: ## 构建 Go 后端 Docker 镜像
	docker build -f deploy/docker/server/Dockerfile -t opsmind-server server/

docker-web: ## 构建 Next.js 前端 Docker 镜像
	docker build -f deploy/docker/web/Dockerfile -t opsmind-web web/
