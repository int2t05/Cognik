...
func TestMessageRepo_Create(t *testing.T) {
	...
	repo := message.NewMessageRepo(db)
...
func TestMessageRepo_ListByUser(t *testing.T) {
	...
	msgs, total, err := repo.ListByUser(ctx, 1, 1, 10, message.MessageFilter{})
	if err != nil {
...
func TestMessageRepo_ListByUser_UnreadFilter(t *testing.T) {
	...
	isRead := false
	msgs, total, err := repo.ListByUser(ctx, 1, 1, 10, message.MessageFilter{IsRead: &isRead})
	if err != nil {
		t.Fatalf("ListByUser 失败: %v", err)
	}
	if total != 1 {
...
func TestMessageRepo_ListByUser_TypeFilter(t *testing.T) {
	...
	msgs, total, err := repo.ListByUser(ctx, 1, 1, 10, message.MessageFilter{Type: "ticket_supplement"})
	if err != nil {
		t.Fatalf("ListByUser 失败: %v", err)
	}
	if total != 1 {
...
func TestMessageRepo_MarkAsRead(t *testing.T) {
	...
	// 验证已读
	isRead := true
	msgs, _, _ := repo.ListByUser(ctx, 1, 1, 10, message.MessageFilter{IsRead: &isRead})
	found := false
	for _, m := range msgs {
		if m.ID == id {
			found = true
