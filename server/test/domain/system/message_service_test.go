...
func setupMessageService(t *testing.T) *message.MessageService {
	t.Helper()
	...
	repo := message.NewMessageRepo(msgSvcDB)
	return message.NewMessageService(repo)
}
...
func TestMessageService_CountUnreadCacheInvalidatesOnMarkAsRead(t *testing.T) {
	...
	repo := message.NewMessageRepo(msgSvcDB)
	svc := message.NewMessageServiceWithCacheTTL(repo, time.Minute)
...
func TestMessageService_ListMessages(t *testing.T) {
	...
	msgs, total, err := svc.ListMessages(bgCtx, 1, 1, 10, message.MessageFilter{})
	if err != nil {
		t.Fatalf("期望无错误, got %v", err)
	}
	if total != 3 {
...
func TestMessageService_ListMessages_Pagination(t *testing.T) {
	...
	msgs, total, err := svc.ListMessages(bgCtx, 1, 1, 2, message.MessageFilter{})
	if err != nil {
		t.Fatalf("期望无错误, got %v", err)
	}
	if total != 5 {
