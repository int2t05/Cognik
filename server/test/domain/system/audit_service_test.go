...
func setupAuditServiceTestDB(t *testing.T) *gorm.DB {
	...
	db, err := database.Init(config.DatabaseConfig{
		...
		User: envOrDefault("TEST_DB_USER", "cognik"), Password: envOrDefault("TEST_DB_PASSWORD", "cognik_dev"),
		DBName: envOrDefault("TEST_DB_NAME", "cognik_test"), SSLMode: envOrDefault("TEST_DB_SSLMODE", "disable"),
	})
	if err != nil {
