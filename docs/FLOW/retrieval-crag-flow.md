...
运维团队向知识库提问，系统从已发布文章中检索相关片段，交给大模型生成带引用的回答。检索不准则回答跑偏，检索太慢则体验差。本文档描述检索如何又快又准，以及在检索不足时如何自动识别并补救。
...
flowchart TD
    ...
    PAR --> VEC["向量检索<br/>query embed 缓存<br/>→ pgvector cosine<br/>ef_search=100, halfvec(1024)"]
