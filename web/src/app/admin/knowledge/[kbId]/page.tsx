        ...
        <>
          <DataTable
            columns={[
              ...
              { id: 'source_type_text', header: () => <TableFilterHeader label="来源" value={sourceType} options={ARTICLE_SOURCE_OPTIONS} onChange={(v) => { setSourceType(v); setPage(1); }} />, cell: ({ row }) => <span className="text-fine">{row.original.source_type === 1 ? '手动' : '上传'}</span> },
              ...
              { accessorKey: 'process_status', header: () => <TableFilterHeader label="处理" value={processStatus} options={ARTICLE_PROCESS_OPTIONS} onChange={(v) => { setProcessStatus(v); setPage(1); }} />, cell: ({ row }) => row.original.process_status ? <StatusBadge type="process" status={row.original.process_status} /> : '—' },
